...
export function FinanceDashboard() {
  ...
  // Don't render anything until client is hydrated to prevent hydration mismatch
  if (!mounted) {
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="rounded-xl border border-[#2a2825] bg-[#1e1c1a] p-4 animate-pulse h-24" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-[#2a2825] bg-[#1e1c1a] p-5">
        <p className="text-[11.5px] text-[#5c5248] font-medium mb-4">MONTHLY SPEND TREND</p>
        {mounted && (
        ...
        <div className="flex gap-6 mt-4 text-[10px]">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[#5c5248]">Actual Spend</span>
          </div>
          <div className="flex items-center gap-2">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 pt-4">
        <p className="text-[11px] text-[#3a3430]">Financial data updated monthly. Next update scheduled in 3 days.</p>
      </div>
    </div>
  );
}
